...
const applyFilter =() =>{
  if(Spacility){
    ...
    console.log("inside Dermotologist",Spacility)
  }
  else{
     ...
     <div className='flex flex-col sm:flex-row items-start gap-5 mt-5' >
      <button className={`py-1 px-3 border rounded text-sm transition-all sm:hidden ${showFilter?'bg-primary text-white' :''} `} onClick={() => setshowFilter(prev =>!prev)}>Filters</button>
        <div className={`flex-col gap-4 text-sm text-gray-600 ${showFilter ? 'flex':'hidden sm:flex'}`}>
              ...
              <p onClick={()=>Spacility === 'Dermotologist' ? navigate('/doctors'):navigate('/doctors/Dermotologist')} className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer           ${Spacility === 'Dermotologist'?"bg-indigo-100 text-black": " "} ` }>Dermotologist</p>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 w-full gap-y-6 gap-5'>
            {
              FilterDoc.map((item,index) =>(
                 ...
                 <div onClick={() => navigate(`/appointments/${item._id}`)} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:-translate-y-2 transition-all duration-500'>
                       ...
                       {console.log("Hii i am Printing item",item)}
